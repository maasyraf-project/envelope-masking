...
% link to spatial anechoic (unvocoded stimuli)
audioANDir = strcat(pwd,'\stimuli\unvocoded\UniS_Anechoic_BRIR_16k');

% create directory path for spatialized reverberant stimmuli
audioOutputDir = strcat(pwd,'\stimuli\enhanced-env-masking\',irName, '\', vocoder_type, '\');
...
if ~exist(refDir, 'dir')
    error('Reference directory is not exist!');
end

% create directory path for result
resultsDir = strcat(pwd,'\results\enhanced-env-masking\', irName, '\', vocoder_type, '\');
...
%         end
        ...
        % import anechoic audio (unvocoded)
        [audioAn, fs] = audioread(fullfile(audioANDir, OutputFilenames));

        % create reverberant audio (unvocoded)
        audioRev = [conv(squeeze(ir_left), audioInput) ...
               conv(squeeze(ir_right), audioInput)];

        audioRev = audioRev(1:length(audioAn),:);

        % make binaural stimuli
        audioOutput = [CI_Sim_Enh_Left(audioRev(:,1), audioAn(:,1), target_fs, vocoder_type) ...
               CI_Sim_Enh_Right(audioRev(:,2), audioAn(:,2), target_fs, vocoder_type)];
